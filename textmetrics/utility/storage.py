...
# local
from common import References, Candidates
...
def save(references: References, candidates: Candidates) -> None:
    ...
    for i, rCorpus in enumerate(references['corpora'].values()):
        rCorpus['tmpfile'] = os.path.join(
            references['tmpdir'], 'reference.{}.001.txt'.format(REF_LABELS[i]))
